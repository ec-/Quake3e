...
====================
*/
static void CL_WriteGamestate( bool initial )
{
	...
	for ( i = 0; i < MAX_GENTITIES ; i++ ) {
		...
		MSG_WriteDeltaEntity( &msg, &nullstate, ent, true );
	}
...
static void CL_EmitPacketEntities( clSnapshot_t *from, clSnapshot_t *to, msg_t *msg, entityState_t *oldents ) {
	...
	while ( newindex < to->numEntities || oldindex < from_num_entities ) {
		...
		if ( newnum < oldnum ) {
			...
			MSG_WriteDeltaEntity (msg, &cl.entityBaselines[newnum], newent, true );
			newindex++;
			continue;
		}
...
static void CL_CompleteRecordName(const char *args, int argNum )
{
	if ( argNum == 2 )
	{
		...
		Field_CompleteFilename( "demos", demoExt, true, FS_MATCH_EXTERN | FS_MATCH_STICK );
	}
}
...
static int CL_WalkDemoExt( const char *arg, char *name, int name_len, fileHandle_t *handle )
{
	...
	*handle = FS_INVALID_HANDLE;
	...
	while ( demo_protocols[ i ] )
	{
		...
		FS_BypassPure();
		FS_FOpenFileRead( name, handle, true );
		FS_RestorePure();
...
====================
*/
static bool CL_DemoNameCallback_f( const char *filename, int length )
{
...
static void CL_CompleteDemoName(const char *args, int argNum )
{
	if ( argNum == 2 )
	{
		...
		Field_CompleteFilename( "demos", "." DEMOEXT "??", false, FS_MATCH_ANY | FS_MATCH_STICK );
...
static void CL_PlayDemo_f( void ) {
	...
	if ( ext_test && !Q_stricmpn(ext_test + 1, DEMOEXT, ARRAY_LEN(DEMOEXT) - 1) )
	{
		...
		if ( demo_protocols[ i ] || protocol == com_protocol->integer  )
		{
			...
			FS_BypassPure();
			FS_FOpenFileRead( name, &hFile, true );
			FS_RestorePure();
		}
		else
		{
	...
	if ( FS_FOpenFileRead( name, &clc.demofile, true ) == -1 )
	{
		// drop this time
...
static bool CL_RestoreOldGame( void )
{
	if ( cl_oldGameSet )
	{
		...
		FS_ConditionalRestart( clc.checksumFeed, true );
		return true;
	}
	return false;
}
...
static void CL_ForwardToServer_f( void ) {
	...
	CL_AddReliableCommand( Cmd_ArgsFrom( 1 ), false );
}
...
void CL_Disconnect_f( void ) {
	...
	if ( cls.state != CA_DISCONNECTED && cls.state != CA_CINEMATIC ) {
		if ( (uivm && uivm->callLevel) || (cgvm && cgvm->callLevel) ) {
			...
			if ( !CL_Disconnect( false ) ) { // restart client if not done already
				CL_FlushMemory();
			}
			if ( uivm ) {
...
void CL_InitDownloads( void ) {
	...
	if ( cl_mapAutoDownload->integer && ( !(clc.sv_allowDownload & DLF_ENABLE) || clc.demoplaying ) )
	{
		...
		if ( FS_FOpenFileRead( bsp, NULL, false ) == -1 )
		{
			if ( CL_Download( "dlmap", mapname, true ) )
			{
...
static void CL_CheckForResend( void ) {
	...
	case CA_CHALLENGING:
		...
		if ( strlen( info ) > MAX_USERINFO_LENGTH - 64 ) {
			infoTruncated |= Info_RemoveKey( info, "xp_name" ) ? true : false;
			infoTruncated |= Info_RemoveKey( info, "xp_country" ) ? true : false;
		}
...
static bool CL_ConnectionlessPacket( const netadr_t *from, msg_t *msg ) {
	...
	if ( !Q_stricmp(c, "echo") ) {
		...
		if ( (fromserver = NET_CompareAdr( from, &clc.serverAddress )) != false || NET_CompareAdr( from, &rcon_address ) ) {
...
static void CL_CheckUserinfo( void ) {
	...
	if ( cvar_modifiedFlags & CVAR_USERINFO )
	{
		...
		CL_AddReliableCommand( va( "userinfo \"%s\"", info ), false );
	}
}
...
static void CL_CompleteVideoName(const char *args, int argNum )
{
	if ( argNum == 2 )
	{
		Field_CompleteFilename( "videos", ".avi", true, FS_MATCH_EXTERN | FS_MATCH_STICK );
	}
}
...
bool CL_GetModeInfo( int *width, int *height, float *windowAspect, int mode, const char *modeFS, int dw, int dh, bool fullscreen )
{
	const	vidmode_t *vm;
	float	pixelAspect;
...
bool CL_Download( const char *cmd, const char *pakname, bool autoDownload )
{
	char url[MAX_OSPATH];
	char name[MAX_CVAR_VALUE_STRING];
	const char *s;
...
static void CL_Download_f( void )
{
	...
	CL_Download( Cmd_Argv( 0 ), Cmd_Argv( 1 ), false );
}
#endif // USE_CURL
