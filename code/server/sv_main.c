...
bool SVC_RateLimitAddress( const netadr_t *from, int burst, int period ) {
	...
	return bucket ? SVC_RateLimit( &bucket->rate, burst, period ) : true;
}
...
void SV_TrackCvarChanges( void )
{
	...
	Cvar_ResetGroup( CVG_SERVER, false );
	...
	for ( i = 0, cl = svs.clients; i < sv_maxclients->integer; i++, cl++ ) {
		if ( cl->state >= CS_CONNECTED ) {
			SV_UserinfoChanged( cl, false, false ); // do not update userinfo, do not run filter
		}
	}
}
