...
static void SV_Map_f( void ) {
	...
	FS_BypassPure();
	len = FS_FOpenFileRead( expanded, NULL, false );
	FS_RestorePure();
	if ( len == -1 ) {
...
static void SV_MapRestart_f( void ) {
	...
	for ( i = 0 ; i < sv_maxclients->integer ; i++ ) {
		...
		denied = GVM_ArgPtr( VM_Call( gvm, 3, GAME_CLIENT_CONNECT, i, false, isBot ) );
		if ( denied ) {
...
static void SV_CompleteMapName( const char *args, int argNum ) {
	if ( argNum == 2 ) 	{
		if ( sv_pure->integer ) {
			Field_CompleteFilename( "maps", "bsp", true, FS_MATCH_PK3s | FS_MATCH_STICK );
		} else {
			Field_CompleteFilename( "maps", "bsp", true, FS_MATCH_ANY | FS_MATCH_STICK );
		}
	}
}
